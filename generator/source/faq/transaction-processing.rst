...
When an illegal transaction reaches the transaction processor, and it throws an InvalidTransaction exception, an endless loop is started where the validator repeatedly submits the same invalid transaction to the transaction processor.
Why does this happen?  Is there a fix for this?
----------------------------------------------------------
