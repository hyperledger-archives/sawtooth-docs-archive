...
<h2> What is the difference between Sawtooth, Sawtooth Lake, and Hyperledger
Sawtooth?</h2>
...
<h2> Does Sawtooth focus on developing blockchain solutions for sustainable
fishing?</h2>
...
<h2> What\'s the difference between the `sawtooth`, `sawadm`, `sawnet`, and
`sawset` commands?</h2>
...
<h2> What does `Failed to reach common ancestor` mean
from `sawnet compare-chains`?</h2>
...
<h2>Does Sawtooth restore state when a peer restarts or when a peer is
out-of-sync with the network?</h2>

Yes.

<h2> When content at an address is changed several times by the transactions in
a block, what appears in the state (Merkle Tree)?</h2>
...
<h2> In order to create a Sawtooth application, do I need to clone and modify
the entire `sawtooth-core` repository?</h2>
