...
:   Also known as PoET Simulator. PoET with a simulated SGX environment.
    Provides CFT similar to some other blockchains. Requires
    poet-validator-registry TP. Runs on any processor (does not require
    Intel or SGX). Has Crash Fault Tolerance (CFT), but is not Byzantine
    Fault Tolerant (BFT)

PoET SGX (name \"PoET\", version 0.1)

:   Takes advantage of SGX in order to provide consensus with Byzantine
    ...
    (a processor supporting SGX). Currently supported in Sawtooth 1.0
    only.
...
<h2>For PoET CFT (PoET Simulator), should I generate my own
`simulator_rk_pub.pem` file or do I use the one in `/etc/sawtooth/` ?</h2>
...
<h2> Can my Sawtooth network have validators with a mixture of PoET SGX and
PoET CFT?</h2>
...
simulates the SGX environment and provides CFT. That said, PoET SGX is
...
<h2> I get this error during PoET SGX registration:
"Machine requires update (probably BIOS) for SGX compliance."</h2>
...
<h2>Why does PoET use exponentially-distributed random variable function
instead of a uniform function?</h2>
...
<h2> I run `sudo -u sawtooth poet registration create . . .` and get
`Permission denied: 'poet_genesis.batch'` error</h2>
...
<h2> What does `Failed to create wait certificate: Cannot create wait
certificate because timer has timed out` mean?</h2>
